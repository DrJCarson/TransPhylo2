...
inferTTree2 <- function(ptree,
  ...
  mcmc.cov.rho <- diag(rep(0.1 ^ 2 * as.numeric(update.rho), rho.dim))
  ...
  for (i in 1:mcmcIterations) {
    ...
    if (update.rho) {
      ...
      if (rho.dim == 1) {
        ...
        parms.prop.rho <- MASS::mvrnorm(1, mu = parms.curr.rho, Sigma = ss.lam.rho * ss.c.rho * mcmc.cov.rho)

      }

      ss.u.rho <- log(runif(1))

      if (all(parms.prop.rho >= 0) & all(parms.prop.rho <= 1)) {

        ext.rho2 <- parms.prop.rho[rho.demes]
      ...
      if (i == 1) {
        ...
        mcmc.cov.rho <- (1 / (ss.v0.rho + ss.d.rho + 3)) * (parms.init.rho %*% t(parms.init.rho) +
                                                              parms.curr.rho %*% t(parms.curr.rho) +
                                                              (ss.v0.rho + ss.d.rho + 1) * mcmc.cov.rho -
                                                              2 * mcmc.mu.rho %*% t(mcmc.mu.rho))
        ...
        mcmc.cov.rho <- (1 / (i - ss.f[i] + ss.v0.rho + ss.d.rho + 2)) *
          ((i - ss.f[i] + ss.v0.rho + ss.d.rho + 1) * mcmc.cov.rho +
             parms.curr.rho %*% t(parms.curr.rho) +
             (i - ss.f[i]) * mcmc.mu.rho %*% t(mcmc.mu.rho) -
             (i - ss.f[i] + 1) * mcmc.mu.new.rho %*% t(mcmc.mu.new.rho))
        ...
        mcmc.cov.rho <- mcmc.cov.rho + (1 / (i - ss.f[i] + ss.v0.rho + ss.d.rho + 2)) *
          (parms.curr.rho %*% t(parms.curr.rho) - parms.rem.rho %*% t(parms.rem.rho) +
             (i - ss.f[i] + 1) * (mcmc.mu.rho %*% t(mcmc.mu.rho) - mcmc.mu.new.rho %*% t(mcmc.mu.new.rho)))
